...
def _build_provenance_for_thread(
    ...
    LangGraph emits multiple checkpoints per turn (a human-step snapshot and
    one or more ai-step snapshots with the same message count). These same-
    message-count snapshots are *siblings* within a turn, not a parent-child
    chain. We collapse each turn to its final ai checkpoint (the last snapshot
    at a given message count) so the DAG has one node per turn, with a clean
    parent = previous turn's node.
    """
    ...
    # Keep only ai-ending checkpoints, then collapse same-nmsg siblings to
    # the LAST one (the turn's final state). Dict keyed by nmsg preserves
    # insertion (chronological) order in Python 3.7+.
    by_turn: dict[int, dict[str, Any]] = {}
    for cp in raw_checkpoints:
        if not _has_assistant(cp):
            continue
        by_turn[_nmsg(cp)] = cp  # later same-nmsg overwrites → final state
